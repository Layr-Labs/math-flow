...
EXPECTED_CASES = {
    2: (43, 17, ("case-color-2-extra-17.lrat.gz",)),
    3: (55, 5, ("case-color-3-extra-5.lrat.gz",)),
    4: (38, 22, ("case-color-4-extra-22.lrat.gz",)),
    5: (32, 28, ("case-color-5-extra-28.lrat.gz",)),
    6: (35, 25, ("case-color-6-extra-25.lrat.gz",)),
}
CASE_KEYS = {
    "newColor",
    "blockerPairs",
...
MAX_COMPRESSED_PROOF_BYTES = 90 * 1024 * 1024
MAX_UNCOMPRESSED_SHARD_BYTES = 384 * 1024 * 1024
MAX_UNCOMPRESSED_PROOF_BYTES = 600 * 1024 * 1024
MAX_TOTAL_COMPRESSED_PROOF_BYTES = 250 * 1024 * 1024

EXPECTED_PROOF_GENERATION = {
    "solver": "CaDiCaL 3.0.1",
    "format": "text-lrat-gzip",
    "compression": ["gzip", "-n", "-9", "-c"],
    "baseOptions": ["--unsat", "--lrat=true", "--binary=false", "--checkproof=2"],
    "caseOptions": {
        "5": ["--shuffle=true", "--shufflerandom=true", "--seed=1", "-t", "300"],
        "6": ["--shuffle=true", "--shufflerandom=true", "--seed=1", "-t", "300"],
    },
    "normalization": {
        "tool": "normalize_lrat.py",
        "removedHints": {
            "2": {"satisfied": 0, "afterConflict": 0},
            "3": {"satisfied": 0, "afterConflict": 0},
            "4": {"satisfied": 0, "afterConflict": 0},
            "5": {"satisfied": 4, "afterConflict": 0},
            "6": {"satisfied": 0, "afterConflict": 0},
        },
    },
    "coreExtraction": {
        "tool": "trim_lrat_core.py",
        "format": "transitive-rup-core-from-final-empty-clause",
        "cases": {
            "2": {"inputSha256": "bd7c01601a7b86541c61666934f47277920c2ceecf932a4e69e035a39ac6beb6", "inputAdditions": 110251, "inputDeletions": 34103, "keptAdditions": 47973},
            "3": {"inputSha256": "9d884973ee4b0c810df13bc5615f0d94b67e1cbb1f803d8a04358d8612286305", "inputAdditions": 1808, "inputDeletions": 30, "keptAdditions": 414},
            "4": {"inputSha256": "7356572351dfd56c2909d6921bf56b7aadf0b6018ea87438aeb94c75bb237820", "inputAdditions": 252656, "inputDeletions": 64886, "keptAdditions": 108901},
            "5": {"inputSha256": "6fabb1dbcbf7ac03608601b6b0cc78e2354a8e96a42cc06cb2bb561cb515dcf1", "inputAdditions": 1555164, "inputDeletions": 306754, "keptAdditions": 683703},
            "6": {"inputSha256": "010693fe1d7267e1582b204325e5c31c0ebbc4a705d6510c339197fc1467f605", "inputAdditions": 1540263, "inputDeletions": 265432, "keptAdditions": 691722},
        },
    },
    "postExtractionReplay": {
        "tool": "normalize_lrat.py",
        "removedHints": {
            "2": {"satisfied": 0, "afterConflict": 0},
            "3": {"satisfied": 0, "afterConflict": 0},
            "4": {"satisfied": 0, "afterConflict": 0},
            "5": {"satisfied": 0, "afterConflict": 0},
            "6": {"satisfied": 0, "afterConflict": 0},
        },
    },
}
    ...
    for path in paths:
        ...
        with gzip.open(path, "rb") as stream:
            while chunk := stream.read(1024 * 1024):
                ...
                if shard_size > MAX_UNCOMPRESSED_SHARD_BYTES:
                    raise ValueError(f"{path.name} expands beyond the shard byte bound")
                if logical_size > MAX_UNCOMPRESSED_PROOF_BYTES:
                    raise ValueError("logical proof expands beyond the proof byte bound")
                shard_digest.update(chunk)
                logical_digest.update(chunk)
        records.append((shard_digest.hexdigest(), shard_size))
    return logical_digest.hexdigest(), logical_size, records
...
def verify_checker_controls() -> None:
    """Exercise the ordered-RUP core on one valid and three invalid toys."""
    toy_clauses: list[tuple[int, ...] | None] = [None, (1,), (-1,)]

    # Assuming the empty clause false, the two opposing units conflict.
    verify_rup((), [1, 2], toy_clauses, 3)

    rejected = (
        ((), [1]),       # an incomplete propagation chain
        ((), [1, 1]),    # a satisfied repeated reason, not another unit
        ((-1,), [1]),    # the proposed clause is not RUP from clause 1
    )
    for clause, hints in rejected:
        try:
            verify_rup(clause, hints, toy_clauses, 3)
        except ValueError:
            continue
        raise AssertionError("ordered-RUP checker accepted an invalid control")
...
def verify_lrat(
        ...
        with gzip.open(proof_path, "rt", encoding="ascii", newline="") as stream:
            for raw_line in stream:
                ...
                if not raw_line.endswith("\n") or raw_line.strip() != raw_line[:-1]:
                    raise ValueError(
                        f"noncanonical LRAT line {lines} in {proof_path.name}"
                    )
                ...
                if len(tokens) >= 3 and tokens[1] == "d":
                    ...
                    for deleted_id in deleted_ids:
                        if (
                            ...
                            or deleted_id >= len(clause_table)
                        ):
                            raise ValueError(
                ...
                if any(
                    literal == 0 or abs(literal) > variable_count for literal in clause
                ):
                    raise ValueError(
                        f"LRAT clause literal outside the CNF variables at line {lines}"
                    )
                if len(set(clause)) != len(clause):
                ...
                if any(hint <= 0 for hint in hints):
                    raise ValueError(
                        "this checker accepts the committed RUP-only LRAT subset"
                    )
                ...
                if clause_table[command_id] is not None:
                    raise ValueError(
                        f"LRAT clause identifier {command_id} is already occupied"
                    )
    ...
    if lines == 0 or last_added_clause != ():
        raise ValueError("logical proof does not end by deriving the empty clause")
    return {
        "lines": lines,
        "additions": additions,
        "deletions": deletions,
...
def read_metadata(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("cases.json does not describe the fixed radius-60 problem")
    ...
    if data["proofGeneration"] != EXPECTED_PROOF_GENERATION:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the exact radius-60 nonextension certificate at n=537"
    )
    ...
    if not isinstance(cases, list) or len(cases) != len(EXPECTED_CASES):
        raise ValueError("cases.json must contain exactly five conditioned cases")
    ...
    total_proof_lines = 0
    total_compressed_proof_bytes = 0
    for index, case in enumerate(cases):
        ...
        expected_blockers, expected_extra, expected_proofs = EXPECTED_CASES[color]
        if (
        ...
        proof_files = case["proofFiles"]
        if (
            not isinstance(proof_files, list)
            or [item.get("file") for item in proof_files if isinstance(item, dict)]
            != list(expected_proofs)
        ):
            raise ValueError(f"proof shards for color {color} are not canonical")
        proof_paths: list[Path] = []
        for shard_index, shard in enumerate(proof_files):
            if not isinstance(shard, dict) or set(shard) != SHARD_KEYS:
                raise ValueError(
                    f"proof shard {shard_index} for color {color} is malformed"
                )
            proof_path = args.cases.parent / expected_proofs[shard_index]
            compressed_digest, compressed_size = sha256_file(
                proof_path, maximum_bytes=MAX_COMPRESSED_PROOF_BYTES
            )
            if compressed_digest != shard["compressedSha256"]:
                raise ValueError(
                    f"compressed proof digest mismatch for color {color}"
                )
            if compressed_size != exact_int(
                shard["compressedBytes"], "compressedBytes"
            ):
                raise ValueError(f"compressed proof size mismatch for color {color}")
            total_compressed_proof_bytes += compressed_size
            proof_paths.append(proof_path)

        payload_digest, payload_size, payload_records = inspect_gzip_payloads(
            proof_paths
        )
        if payload_digest != case["logicalProofSha256"] or payload_size != exact_int(
            case["logicalProofBytes"], "logicalProofBytes"
        ):
            raise ValueError(f"logical LRAT payload mismatch for color {color}")
        for shard, (shard_digest, shard_size) in zip(
            proof_files, payload_records, strict=True
        ):
            if shard_digest != shard["uncompressedSha256"] or shard_size != exact_int(
                shard["uncompressedBytes"], "uncompressedBytes"
            ):
                raise ValueError(f"LRAT shard payload mismatch for color {color}")
        proof_stats = verify_lrat(proof_paths, formula, variable_count)
        for field, key in (
            ("lines", "proofLines"),
    ...
    if total_compressed_proof_bytes > MAX_TOTAL_COMPRESSED_PROOF_BYTES:
        raise ValueError("compressed proof bundle exceeds the repository byte bound")

    excluded_by_count = {
        color for color, count in blocker_counts.items() if count > RADIUS
    }
    if excluded_by_count != {1}:
    ...
    print(
        ...
        f"five RUP-only LRAT proofs ({total_proof_lines} lines, "
        f"{total_compressed_proof_bytes} compressed bytes): every valid "
        f"coloring of 1..537 differs from the fixed baseline on at least 61 "
        f"of integers 1..536"
    )
    return 0
