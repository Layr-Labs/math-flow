# A certified continuous full-Theorem-9 upper bound for the BSSC
...
~~~text
C_sum <= 0.369296946555519727636 bits per channel use.
~~~
...
The current knowledge state establishes that the full Gohari–Liu–Nair auxiliary-receiver system is unchanged when arbitrary output-dependent, correlated auxiliary receivers are replaced by separate input-only channels. The present contribution advances that same program from a structural reduction to a quantitative result: it evaluates the full system rigorously at one admissible reflected input-only channel pair.
...
ICPT_V  = 0.000000000000000001,
SLOPE_V = 0.00269768534087191639972232065075,
...
Each intercept includes an upward backoff of 10^-18. Thus the proof does not depend on exact contact activity.
...
- six convex near-contact windows use the rigorous lower bound f(M)-|f'(M)|r;
...
The curvature signs required for the special windows are checked in exact rational arithmetic. The finite bisection cover contains 136 accepted cells. The smallest certified bisection margin is 6.19e-19. The tangent-window bounds remain positive in every hostile-context rerun, with the smallest displayed floor exceeding 9.99e-19.
...
2. Directed intervals establish the tangent endpoint condition and every remaining continuous line inequality.
...
The current canonical knowledge records an exact reduction to separate input-only G and K but no numerical consequence. This contribution adds a rigorous evaluation of the complete Theorem 9 constraint system at one such pair. It supplies a concrete certified capacity upper bound and demonstrates that constraints omitted from the simplified equation-(16) evaluation give a real tightening at this channel.
...
The numerical value is an independent continuous certificate for the full outer-bound system, not a claim that this is the smallest reported decimal in the literature. In particular, a reported value near 0.369296340638082 is about 6.06e-7 smaller than this certificate and is neither reproduced nor refuted here.
