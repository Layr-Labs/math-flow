...
where the dual-feasibility conditions certified here are
  ...
  (D1)  every inner affine majorant used dominates its curve on ALL of [0,1]:
          ...
          group B: the backed-off chord lines ell_V >= f_V^B and
                   ell_U >= f_U^B on [0,1]      (interval bisection);
...
Nothing here assumes exact contacts or active sets: all frozen decimals are
backed off (BACKOFF = 1e-18 added to every line intercept) so each certified
inequality is strict with comfortable margins.
...
SLOPE_V = D("0.00269768534087191639972232065075")          # nearest-30
ICPT_V = BACKOFF
...
def phase3_tangent_and_endpoint(gaps: Gaps) -> None:
    ...
    tangent_bound("C2c window @M_C0      ", gaps.gap_c2, gaps.gap_c2_p,
                  M_C0, DELTA_WIN)
    tangent_bound("BV  window @M_BV      ", gaps.gap_bv, gaps.gap_bv_p,
                  M_BV, DELTA_WIN)
    tangent_bound("BU  window @M_BU      ", gaps.gap_bu, gaps.gap_bu_p,
                  M_BU, DELTA_WIN)
...
def phase4_bisection(gaps: Gaps) -> int:
    ...
    segments = (
        ...
        ("BV-left ", gaps.gap_bv, gaps.gap_bv_p, D(0),
         down_sub(M_BV, DELTA_WIN), True),
        ("BV-right", gaps.gap_bv, gaps.gap_bv_p, up_add(M_BV, DELTA_WIN),
         D(1), True),
        ("BU-left ", gaps.gap_bu, gaps.gap_bu_p, D(0),
         down_sub(M_BU, DELTA_WIN), True),
        ("BU-right", gaps.gap_bu, gaps.gap_bu_p, up_add(M_BU, DELTA_WIN),
         D(1), True),
    )
