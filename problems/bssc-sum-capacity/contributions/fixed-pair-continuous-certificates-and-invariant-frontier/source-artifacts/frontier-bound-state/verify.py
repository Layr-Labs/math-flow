#!/usr/bin/env python3
"""Outward certificate for C_sum <= 0.369296945969202842443."""
...
IV0 = D("1e-18")
WIN = D("0.0625")
OLD = D("0.36929694655551972563539254207215942386102502532943886683678450695288358384488468")
...
# Six accepted Theorem-9 rows.  A term is (group, MI-kind, channel, sign).
GA, GB, GC = range(3)
Y, Z, G, K = range(4)
    ...
    while stack:
        ...
        if value.lo > 0:
            ...
            if endpoint_derivative or (x>0 and y<1):
                width=UP.multiply(UP.subtract(y,x),D(".5"))
...
def certify():
    ...
    need(value.hi < OLD, "no strict frontier improvement")
...
def main():
    ...
    print("U =",reference)
    print("margin below prior certified endpoint =",DOWN.subtract(OLD,reference.hi))
