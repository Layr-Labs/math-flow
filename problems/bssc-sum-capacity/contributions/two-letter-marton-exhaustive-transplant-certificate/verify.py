...
EXPECTED_LHALF = Decimal(
    "0.545190401132220579101772556649110871695949196617116813626380000916798230498"
)
EXPECTED_ENDPOINT_Y = Decimal(
    "0.546525784279605207250945323672001290387190235227857315561583218760711125016"
)
EXPECTED_ENDPOINT_Z = Decimal(
    "0.543855017984835950952599789626220453004708158006376311691176783072885335980"
)
...
def contains_decimal(
    ...
    value: Decimal,
    denominator_factor: int = 1,
) -> bool:
    scaled = value * D(denominator_factor * SCALE)
    return D(interval[0]) <= scaled <= D(interval[1])
...
def check_metadata() -> None:
    ...
    need(
        ...
        "sole valid threshold dependency",
    )
...
def main() -> None:
    ...
    need(
        contains_decimal(candidate, EXPECTED_LHALF, denominator_factor=2),
        "known Lhalf orientation",
    )
    need(contains_decimal(endpoints[0], EXPECTED_ENDPOINT_Y),
         "known Y endpoint orientation")
    need(contains_decimal(endpoints[1], EXPECTED_ENDPOINT_Z),
         "known Z endpoint orientation")
    ...
    print("PASS: exhaustive directed two-law BSSC transplant certificate")
