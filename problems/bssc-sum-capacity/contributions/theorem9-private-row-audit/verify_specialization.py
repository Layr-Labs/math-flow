...
1. ``theorem9_spec.json`` is a direct term-by-term transcription of Theorem 9.
2. ``make_path_rows`` constructs the local L=3 rows from generic path formulas.

After setting R0=0, minima in the source are expanded into scalar rows and the
two interval side conditions are split into four nonnegative slacks.  Both
constructions are normalized only with the mutual-information chain rules
I(U,W;A)=I(W;A)+I(U;A|W) and its V analogue, then compared exactly.
No floating point arithmetic, optimizer, or network request is used.
"""
...
def verify_pdf(path: Path, source: dict[str, object]) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--source-pdf",
        type=Path,
        help="optional downloaded official PDF; verify its size and SHA-256",
    )
    ...
    if args.source_pdf is not None:
        verify_pdf(args.source_pdf, source)
    ...
    print("PASS: (19a)-(19p), both side conditions, and all 30 private-message rows agree exactly")
