...
| General approving reviews | `0` | Ordinary contributions and direction events are permissionless after validation. |
...
required status check and CODEOWNER semantics. A `false` CODEOWNER field in the
repository ruleset does not cancel the classic protection; effective rules are
additive.
...
condition is exactly `refs/heads/main`. A status check named "Admin admission
approval" is not a GitHub approving review; these are separate mechanisms.
