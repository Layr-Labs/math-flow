...
from math_flow.bssc_research_v5_producer import plan_bssc_research_v5_frontier
...
PROJECTION_PATH = ROOT / "protocol/runtime/openrouter-research-v5-projection.json"
...
class BSSCResearchV5ProducerTests(unittest.TestCase):
    def test_initial_frontier_starts_from_zero_and_materializes_one_subject(self) -> None:
        with tempfile.TemporaryDirectory() as directory_value:
            ...
            self.assertEqual(plan["projection"]["id"], "openrouter-research-v5")
    ...
    def test_state_v3_bundle_advances_exactly_one_frontier(self) -> None:
        with tempfile.TemporaryDirectory() as directory_value:
            ...
            initial = plan_bssc_research_v5_frontier(
                ROOT,
            ...
            following = plan_bssc_research_v5_frontier(
                ROOT,
                projection_root=projection_root,
                scheduler_file=scheduler,
    ...
    def test_cli_and_workflow_use_the_serial_v7_route(self) -> None:
        with tempfile.TemporaryDirectory() as directory_value:
            directory = Path(directory_value)
        ...
        self.assertIn(
            "openrouter-research-v5 must run through project-research-v5-serial.yml",
            generic,
        )

    def test_runtime_candidate_is_identical_to_the_admitted_projection(self) -> None:
        admitted = json.loads(
            (ROOT / "protocol/projections/openrouter-research-v5.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(PROJECTION, admitted)
