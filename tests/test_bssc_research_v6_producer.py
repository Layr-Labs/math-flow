...
from math_flow.bssc_research_v6_producer import plan_bssc_research_v6_frontier
...
PROJECTION_PATH = ROOT / "protocol/runtime/openrouter-research-v6-projection.json"
...
def v8_transport(request: dict[str, object]) -> dict[str, object]:
...
class BSSCResearchV6ProducerTests(unittest.TestCase):
    def test_initial_frontier_starts_from_zero_and_materializes_one_subject(self) -> None:
        with tempfile.TemporaryDirectory() as directory_value:
            ...
            self.assertEqual(plan["projection"]["id"], "openrouter-research-v6")
    ...
    def test_state_v3_bundle_advances_exactly_one_frontier(self) -> None:
        with tempfile.TemporaryDirectory() as directory_value:
            ...
            initial = plan_bssc_research_v6_frontier(
                ROOT,
            ...
            builder_path = (
                ROOT / "protocol/judges/openrouter-hierarchical-research-builder-v8.json"
            )
            ...
            following = plan_bssc_research_v6_frontier(
                ROOT,
                projection_root=projection_root,
                scheduler_file=scheduler,
    ...
    def test_cli_and_workflow_use_the_serial_v8_route(self) -> None:
        with tempfile.TemporaryDirectory() as directory_value:
            directory = Path(directory_value)
        ...
        self.assertIn(
            "openrouter-research-v6 must run through project-research-v6-serial.yml",
            generic,
        )
