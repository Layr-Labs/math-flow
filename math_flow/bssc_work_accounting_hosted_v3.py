...
def _uses_builder_v10(config: Mapping[str, object]) -> bool:
    return config.get("id") == "bssc-work-accounting-hosted-v3"
...
def build_bssc_work_disposition_snapshot(
    ...
    *,
    ...
    # use the pipeline's version-matched submission canonicalizer.
    if _uses_builder_v10(config):
        from .work_accounting_pipeline_v3 import normalize_work_accounting_submission
    else:
    ...
    submissions = {
        ...
        for item in load_bssc_zero_lane_accepted_submissions(
            root, source, rich_claims=_uses_builder_v10(config)
        )
    }
...
def _knowledge_state(
    ...
    state = validate_work_accounting_knowledge_state(value, PROBLEM)
...
def discover_published_research_v10_transition_bundle(
    projection_root: Path,
    *,
    base_knowledge_state: Mapping[str, object],
    submission: AcceptedWorkSubmission,
    ...
    """Find the unique published V10 bundle for an exact base and submission."""

    from .work_accounting_pipeline_v3 import normalize_work_accounting_submission
    from .work_accounting_research_v10 import (
    ...
    object_root = projection_root / "objects" / "knowledge-build"
    if not object_root.is_dir() or object_root.is_symlink():
        raise MathFlowError("published research-v10 object root is missing")
    matches: list[Path] = []
    for prefix in sorted(object_root.iterdir(), key=lambda item: item.name):
        if prefix.is_symlink() or not prefix.is_dir() or not re.fullmatch(
            r"[0-9a-f]{2}", prefix.name
        ...
        for bundle_dir in sorted(prefix.iterdir(), key=lambda item: item.name):
            if bundle_dir.is_symlink() or not bundle_dir.is_dir() or not re.fullmatch(
                ...
                continue
            try:
                transition = load_published_research_v10_transition(
                    bundle_dir,
                    expected_bundle_digest=f"sha256:{bundle_dir.name}",
                    expected_problem=PROBLEM,
                    expected_projection_spec_digest=knowledge_projection_spec_digest,
                ...
                provider = PublishedResearchV10TransitionProvider([transition])
                provider(
                    base_knowledge_state=base_knowledge_state,
                    submission=normalized,
                )
    ...
    if len(matches) != 1:
        raise MathFlowError(
            "expected exactly one published research-v10 transition for the frozen subject"
        )
    return matches[0]
...
def _open_lane(
    ...
    v10_lane = _uses_builder_v10(config)
    submissions = load_bssc_zero_lane_accepted_submissions(
        repository_root, source, rich_claims=v10_lane
    )
    ...
    if current is None:
        knowledge = (
            empty_research_program_state_v3(PROBLEM)
            if v10_lane
            else empty_research_program_state_v2(PROBLEM)
        )
        ...
        initialize = initialize_work_accounting_pipeline
        if v10_lane:
            from .work_accounting_pipeline_v3 import (
                initialize_work_accounting_pipeline as initialize,
            )
        pipeline = initialize(
            store,
            repository_root,
...
def execute_bssc_work_accounting(
    ...
    *,
    ...
    if research_bundle_dir is None:
        ...
        discover = (
            discover_published_research_v10_transition_bundle
            if v10_lane
            else discover_published_research_v6_transition_bundle
        )
        research_bundle_dir = discover(
            projection_root,
    ...
    if v10_lane:
        ...
        builder_provider = PublishedResearchV10TransitionProvider([transition])
        from .work_accounting_pipeline_v3 import (
            advance_work_accounting_pipeline as advance,
        )
    else:
        from .work_accounting_research_v6 import (
            PublishedResearchV6TransitionProvider,
        ...
        advance = advance_work_accounting_pipeline
    expected_profile = work_projection_profile_for_hosted_config(config)
