...
from .bssc_research_v4_producer import (
    load_json_file as load_bssc_research_v4_json,
    plan_bssc_research_v4_frontier,
)
from .bssc_research_v5_producer import (
...
from .bssc_research_v7_producer import (
    load_json_file as load_bssc_research_v7_json,
    plan_bssc_research_v7_frontier,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bssc_research_v4_parser = commands.add_parser(
        "bssc-research-v4-frontier",
        help="materialize the next exact accepted BSSC builder-v6 frontier",
    )
    bssc_research_v4_parser.add_argument("--source", required=True, type=Path)
    bssc_research_v4_parser.add_argument("--projection", required=True, type=Path)
    bssc_research_v4_parser.add_argument(
    ...
    bssc_research_v5_parser = commands.add_parser(
        "bssc-research-v5-frontier",
        help="materialize the next exact accepted BSSC builder-v7 frontier",
    )
    bssc_research_v5_parser.add_argument("--source", required=True, type=Path)
    bssc_research_v5_parser.add_argument("--projection", required=True, type=Path)
    bssc_research_v5_parser.add_argument(
        "--expected-projection-digest",
        help="digest resolved from the separately admitted governed projection",
    )
    bssc_research_v5_parser.add_argument(
    ...
    bssc_research_v7_parser = commands.add_parser(
        "bssc-research-v7-frontier",
        help="materialize the next exact accepted BSSC builder-v9 frontier",
    )
    bssc_research_v7_parser.add_argument("--source", required=True, type=Path)
    bssc_research_v7_parser.add_argument("--projection", required=True, type=Path)
    bssc_research_v7_parser.add_argument(
        "--expected-projection-digest",
        help="digest resolved from the separately admitted governed projection",
    )
    bssc_research_v7_parser.add_argument(
    ...
    bssc_research_v7_parser.add_argument("--output", type=Path)

    bssc_research_v8_parser = commands.add_parser(
        "bssc-research-v8-frontier",
        help="materialize the next exact accepted BSSC builder-v10 frontier",
    )
    bssc_research_v8_parser.add_argument("--source", required=True, type=Path)
    bssc_research_v8_parser.add_argument("--projection", required=True, type=Path)
    bssc_research_v8_parser.add_argument(
        "--expected-projection-digest",
        help="digest resolved from the separately admitted governed projection",
    )
    bssc_research_v8_parser.add_argument(
    ...
    bssc_research_v8_parser.add_argument("--output", type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "validate-tree":
        ...
        elif args.command == "bssc-research-v4-frontier":
            result = plan_bssc_research_v4_frontier(
                root,
                projection_root=args.projection_dir,
                scheduler_file=args.scheduler_file,
                materialization_root=args.materialization_dir,
                replay_source=load_bssc_research_v4_json(
                    args.source, "BSSC research-v4 source"
                ),
                projection=load_bssc_research_v4_json(
                    args.projection, "BSSC research-v4 projection"
                ),
                expected_projection_digest=args.expected_projection_digest,
            )
            _write_json(result, str(args.output) if args.output else None)
            return 0
        elif args.command == "bssc-research-v5-frontier":
            result = plan_bssc_research_v5_frontier(
                root,
                projection_root=args.projection_dir,
                scheduler_file=args.scheduler_file,
                materialization_root=args.materialization_dir,
                replay_source=load_bssc_research_v5_json(
                    args.source, "BSSC research-v5 source"
                ),
            ...
            _write_json(result, str(args.output) if args.output else None)
            return 0
        elif args.command == "bssc-research-v7-frontier":
            result = plan_bssc_research_v7_frontier(
                root,
                projection_root=args.projection_dir,
                scheduler_file=args.scheduler_file,
                materialization_root=args.materialization_dir,
                replay_source=load_bssc_research_v7_json(
                    args.source, "BSSC research-v7 source"
                ),
                projection=load_bssc_research_v7_json(
                    args.projection, "BSSC research-v7 projection"
                ),
                expected_projection_digest=args.expected_projection_digest,
            )
            _write_json(result, str(args.output) if args.output else None)
            return 0
        elif args.command == "bssc-research-v8-frontier":
            from .bssc_research_v8_producer import (
                load_json_file as load_bssc_research_v8_json,
                plan_bssc_research_v8_frontier,
            )

            result = plan_bssc_research_v8_frontier(
                root,
                projection_root=args.projection_dir,
                scheduler_file=args.scheduler_file,
                materialization_root=args.materialization_dir,
                replay_source=load_bssc_research_v8_json(
                    args.source, "BSSC research-v8 source"
                ),
                projection=load_bssc_research_v8_json(
                    args.projection, "BSSC research-v8 projection"
                ),
                expected_projection_digest=args.expected_projection_digest,
            )
            _write_json(result, str(args.output) if args.output else None)
